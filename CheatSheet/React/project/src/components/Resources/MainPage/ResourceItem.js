      ...
      {!isLoading && resources && 
        <>
          {(resources.map((resource) =>
            <div key={resource.id} className="max-w-lg my-4 bg-bgWhiteUI-0 rounded-lg shadow-md hover:shadow-2xl flex flex-col h-full">
              <div className="flex justify-between items-center bg-bgGray-0 py-2">
                <span className="font-semibold text-sm text-bgWhiteUI-0 px-4">Date: {resource.dateTime}</span>
                <div className="flex items-end">
                  <span className="font-semibold text-sm text-bgWhiteUI-0 mr-8 overflow-hidden" >Total likes: {resource.totalLikes}</span>
                </div>
              </div>
